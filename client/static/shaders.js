const vertexShaderSource = `
    ...
    void main(void) {
        mat4 normalMatrix = transpose(inverse(u_ModelViewMatrix));
        v_TransformedNormal = normalMatrix * a_VertexNormal;
...
const fragmentShaderSource = `
    ...
    varying vec4 v_TransformedNormal;
    void main(void) {
        vec3 ambientLight = vec3(0.1, 0.4, 0.5);
        vec3 directionalLightColor = vec3(1.0, 1.0, 0.8);
        vec3 directionalVector = normalize(vec3(0.85, 0.8, 0.75));
        float directional = max(dot(v_TransformedNormal.xyz, directionalVector), 0.0);
